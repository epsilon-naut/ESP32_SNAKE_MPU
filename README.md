...
## Overview
A simple snake game made for the ESP32. Since I didn't have any buttons, only an MPU6050 accelerometer on hand from a previous project, I opted to use tilt controls instead of the traditional four-button setup. Drivers for both the ST7735 (SPI) and MPU6050 (I2C) were written by myself.
...
A simple Github Action for CI was implemented on push to automatically build and check new pushes using [ESP-IDF's own CI Action](https://github.com/espressif/esp-idf-ci-action), with some basic caching of repetitive build files to save time.
